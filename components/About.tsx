    ...
    <section id="tentang" className="border-b border-line bg-bg py-10 md:py-14">
      <div className="mx-auto max-w-6xl px-5 md:px-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <span className="hidden md:inline font-mono text-[10px] tracking-[0.12em] text-zinc-400">DESIGNER & DEVELOPER / SEMARANG</span>
        </div>
          ...
          <div className="col-span-12 md:col-span-4 bg-white p-6 md:p-7 flex flex-col">
            <RevealOnScroll className="flex-1 flex flex-col">
              <div className="flex items-center justify-between">
                <span className="font-mono text-[10px] tracking-[0.14em] text-zinc-400">PROFILE — 2026</span>
                ...
                <div className="mt-4 grid grid-cols-3 gap-2 w-full">
                  {[
                    ...
                    <div key={s.l} className="rounded-xl border border-line bg-muted py-3 text-center">
                      <div className="font-display font-bold text-sm leading-none">{s.v}</div>
                      <div className="font-mono text-[9px] tracking-[0.12em] text-zinc-500 mt-1">{s.l}</div>
                    </div>
                  ))}
                </div>
              </div>
              <div className="mt-6 rounded-xl border border-line bg-muted p-4">
                <p className="font-mono text-[10px] tracking-[0.12em] text-zinc-500">FOCUS</p>
          ...
          <div className="col-span-12 md:col-span-8 bg-muted p-6 md:p-8 flex flex-col gap-6">
            <RevealOnScroll>
              <h2 className="font-display font-bold text-3xl md:text-[32px] leading-[0.95] tracking-[-0.03em] text-ink">
                ...
                <br />
                <span className="text-zinc-400 font-mono font-medium tracking-[-0.02em]">penjualan & kepercayaan.</span>
              </h2>
              ...
              {BENEFITS.map((b, i) => (
                <RevealOnScroll key={b.title} delay={i * 80}>
                  <div className="bg-white p-5 h-full flex flex-col">
                    <div className="flex items-center justify-between">
                      <span className="swiss-num text-2xl text-zinc-200">0{i + 1}</span>
                    ...
                    <span className="font-mono text-[10px] tracking-[0.12em] text-zinc-400 mt-3">0{i + 1} / BENEFIT</span>
                  </div>
                </RevealOnScroll>
              ))}
            </div>
